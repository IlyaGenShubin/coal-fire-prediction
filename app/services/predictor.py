...
def prepare_dataset():
    ...
    stack_life = stack_life.rename(columns={"min": "start", "max": "end"})  # ← КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ
    ...
    if not rows:
        raise ValueError("Не удалось создать ежедневную сетку")
    ...
    for _, f in fires.iterrows():
        start_w = f["fire_start"] - timedelta(days=HORIZON_DAYS)
        end_w = f["fire_start"]
        mask = (
            (daily["Склад"] == f["Склад"]) &
    ...
    if not weather_daily.empty:
        min_w, max_w = weather_daily["date"].min(), weather_daily["date"].max()
...
def train_and_save_model():
    ...
    if y.sum() == 0:
        raise ValueError("Нет пожаров в данных")
...
# === API FUNCTIONS ===
def predict_fire_risk(sklad: int, shtabel: int, date_str: str):
    ...
    x_scaled = SCALER.transform(row[FEATURE_COLS].values)
...
def get_fire_calendar():
    if DAILY_DF is None:
        load_model()
    fires = DAILY_DF[DAILY_DF["цель"] == 1][["Склад", "Штабель", "date"]].to_dict(orient="records")
    return {"fires": fires}
